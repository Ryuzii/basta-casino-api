...
export function playBlackjack(): BlackjackResult {
    // Create and shuffle deck
    const deck: Card[] = [];
    for (const suit of suits) for (const rank of ranks) deck.push(rank + suit);
    // Initial deal
    const playerHand: Hand = [drawCard(deck), drawCard(deck)];
    const dealerHand: Hand = [drawCard(deck), drawCard(deck)];
    // Player auto-stands at 17+
    while (getValue(playerHand) < 17) playerHand.push(drawCard(deck));
    // Dealer stands at 17+
    while (getValue(dealerHand) < 17) dealerHand.push(drawCard(deck));
